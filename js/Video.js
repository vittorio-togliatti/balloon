...
SideScroller.Video.prototype = {
  ...
  create: function() {
    ...
    var tween = this.game.add.tween(this.nens).to( { x: 100 }, this.params.tiempoEntradaNinos, Phaser.Easing.Cubic.Out, true);
  ...
  nens_salen: function(){
    this.texto.kill();
    this.nens.play('run', 5, true);
    var tween = this.game.add.tween(this.nens).to( { x: 650 }, this.params.tiempoSalidaNinos, Phaser.Easing.Cubic.Out, true);
    tween.onComplete.add(this.entraGlobo, this);
    },
    
  entraGlobo: function(){
      this.bkg2 = this.game.add.sprite(640, 0,'img_intro');
      this.intro_globo = this.game.add.sprite(700, -260, 'sheet_intro_globo',0);
      this.intro_globo.animations.add('anim');
      this.intro_globo.play('anim', 5, true);
      
      this.world.bringToTop(this.boton_next);
      
      var tween = this.game.add.tween(this.bkg).to( { x: -640 }, this.params.tiempoEntradaGlobo, Phaser.Easing.Cubic.InOut, true);
      
      var tween = this.game.add.tween(this.bkg2).to( { x: 0 }, this.params.tiempoEntradaGlobo, Phaser.Easing.Cubic.InOut, true);
      
      var tween = this.game.add.tween(this.intro_globo).to( { x: 60 }, this.params.tiempoEntradaGlobo, Phaser.Easing.Cubic.InOut, true);
